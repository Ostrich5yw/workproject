...
layui.define(["laytpl", "form", "util"], function (s) {
    ...
    p.prototype.initOptions = function (t) {
            ...
            for (var x = 0; x < C.length; x++) {
                ...
                if (A.colGroup || A.colspan > 1) {
                    ...
                    for (var v = 0; v < t.cols[y + 1].length; v++) {
                        var H = g.extend({INIT_OK: true}, r, t.cols[y + 1][v]);
        ...
        if ("default" === this.options.toolbar) {
            this.options.toolbar = ["<div>", '   <div class="ew-tree-table-tool-item" title="添加" lay-event="add">', '      <i class="layui-icon layui-icon-add-1"></i>', "   </div>", '   <div class="ew-tree-table-tool-item" title="修改" lay-event="update">', '      <i class="layui-icon layui-icon-edit"></i>', "   </div>", '   <div class="ew-tree-table-tool-item" title="删除" lay-event="delete">', '      <i class="layui-icon layui-icon-delete"></i>', "   </div>", "</div>"].join("")
        }
    ...
    p.prototype.init = function () {
        ...
        if (E.toolbar === false || E.toolbar === undefined) {
            ...
            for (var y = 0; y < E.defaultToolbar.length; y++) {
                ...
                if ("filter" === E.defaultToolbar[y]) {
                    w = {title: "筛选", layEvent: "LAYTABLE_COLS", icon: "layui-icon-cols"}
                } else {
                    if ("exports" === E.defaultToolbar[y]) {
                        w = {title: "导出", layEvent: "LAYTABLE_EXPORT", icon: "layui-icon-export"}
                    } else {
                        if ("print" === E.defaultToolbar[y]) {
                            w = {title: "打印", layEvent: "LAYTABLE_PRINT", icon: "layui-icon-print"}
                        } else {
        ...
        if (E.height) {
            ...
            if (E.height.indexOf("full-") === 0) {
                ...
                A.$tBodyGroup.append(['<style>[lay-filter="', B, '"] .ew-tree-table-box {', "   height: ", m() - z, "px;", "   height: -moz-calc(100vh - ", z, "px);", "   height: -webkit-calc(100vh - ", z, "px);", "   height: calc(100vh - ", z, "px);", "}</style>"].join(""));
            ...
            A.$tHeadGroup.hide();
            var x = {lg: 50, sm: 30, md: 38};
            A.$tBodyGroup.append(['<style>[lay-filter="', B, '"] .ew-tree-table-box:before {', '   content: "";', "   position: absolute;", "   top: 0; left: 0; right: 0;", "   height: " + (x[E.size || "md"] * E.cols.length) + "px;", "   background-color: #f2f2f2;", "   border-bottom: 1px solid #e6e6e6;", "}</style>"].join(""));
            A.$tBody.html(v + D + "<tbody></tbody>")
        }
        c.render("checkbox", B);

        function t(H) {
            var F = H.data("parent"), G;
            if (!F) {
                return
            }
    ...
    p.prototype.bindEvents = function () {
        ...
        w.off("click.tool").on("click.tool", "*[lay-event]", function (z) {
            layui.stope(z);
            var y = g(this);
            layui.event.call(this, h, "tool(" + u.filter + ")", x.call(this, {event: y.attr("lay-event")}))
        });
        c.on("radio(" + u.radioFilter + ")", function (y) {
            ...
            layui.event.call(this, h, "checkbox(" + u.filter + ")", {checked: true, data: z, type: "one"})
        });
        c.on("checkbox(" + u.checkboxFilter + ")", function (C) {
            ...
            v.checkChooseAllCB();
            layui.event.call(this, h, "checkbox(" + u.filter + ")", {checked: B, data: D, type: "more"})
        });
        c.on("checkbox(" + u.chooseAllFilter + ")", function (A) {
            ...
            layui.event.call(this, h, "checkbox(" + u.filter + ")", {checked: z, type: "all"});
            ...
            if (H) {
                ...
                if ("text" === H || "number" === H) {
                    ...
                    F.blur(function () {
                        ...
                        var I = layui.event.call(this, h, "edit(" + u.filter + ")", x.call(this, {value: K, field: G}));
                        if (I === false) {
                ...
                var A = layui.event.call(this, h, "cell(" + u.filter + ")", x.call(this, {td: y, field: G}));
                if (A === false) {
                    layui.stope(C)
                }
            }
        ...
        w.off("dblclick.cellDouble").on("dblclick.cellDouble", "td", function (C) {
            ...
            var y = layui.event.call(this, h, "cellDouble(" + u.filter + ")", x.call(this, {td: D, field: B}));
            if (y === false) {
                layui.stope(C)
            }
        });
            ...
            if ("LAYTABLE_COLS" === y) {
                ...
                if ("LAYTABLE_EXPORT" === y) {
                    ...
                    if ("LAYTABLE_PRINT" === y) {
                        ...
                        layui.event.call(this, h, "toolbar(" + u.filter + ")", {event: y, elem: z})
                    }
                }
            }
        });
    ...
    p.prototype.renderBodyTh = function () {
        ...
        g.each(t.cols, function (x, w) {
            ...
            g.each(w, function (A, z) {
                ...
                var y = '<input type="checkbox" lay-filter="' + v.chooseAllFilter + '" lay-skin="primary" class="ew-tree-table-checkbox"/>';
                if (z.type === "checkbox") {
                    u.push(y)
                } else {
                    u.push(z.title || "")
    ...
    p.prototype.checkStatus = function (t) {
        ...
        this.eachData(function (v, w) {
            if ((t || !w.LAY_INDETERMINATE) && w.LAY_CHECKED) {
                u.push(g.extend({isIndeterminate: w.LAY_INDETERMINATE}, w))
            }
        });
        return u
    };
    ...
    p.prototype.exportData = function (w) {
        ...
        if ("show" === w) {
            ...
            var z = ({csv: "text/csv", xls: "application/vnd.ms-excel"})[w];
    ...
    g(document).on("click", ".ew-tree-table-cell>.layui-table-grid-down", function (w) {
        ...
        if (u.outerWidth() < t) {
            u.children(".ew-tree-table-cell-content").css({"width": t, "max-width": t})
        }
    ...
    function j() {
        ...
        g(".ew-tree-table-cell>.ew-tree-table-cell-content").css({"width": "", "max-width": ""})
    }
